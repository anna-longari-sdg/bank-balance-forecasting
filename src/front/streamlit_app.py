...
def add_edited_drivers():
    ...
    if 0 in st.session_state.value_df["edited_rows"]:
        # Delete the edit from the state dictionary to revert it
        del st.session_state.value_df["edited_rows"][0]
        st.toast("The first row is locked. The edit will be discared.", icon="🚫")
...
for area in st.session_state.eco_anagrafica["ECO_GRP_0_DSC"].unique():
    # Se l'area è quella selezionata, usa 'primary' (colore pieno), altrimenti 'tertiary'
    is_selected = st.session_state.get("selected_area") == area
    st.sidebar.button(
        f"- {area}",
            ...
            with st.spinner("Processing..."):
                ...
                for row_index, row in st.session_state.value_df["edited_rows"].items():
                    for col, new_value in row.items():
                        if row_index > 0 and col not in cols_to_freeze:
                ...
                eco_involved_df = data_loader.eco_df[data_loader.eco_df["ECO_COD"].isin(eco_involved)].reset_index(
                    drop=True
                )
                ...
                forecast_economics(
                    data_loader, forecasting_model_eco, make_plots=False, progress_callback=update_progress, n_jobs=-1
                )
                # Save results in session state and update the economics table with new forecast
                ...
                mask = (
                    (df_rec_forecast_df["ECO_COD"].isin(eco_involved))
                    & (df_rec_forecast_df['DATE_RIF'].dt.strftime("%Y-%m") >= min_month)
                )
                ...
                if VAR_FORECAST == "y_hat_rec":
                    ...
                    df_fit_eco = (
                        st.session_state.rec_fit_df[st.session_state.rec_fit_df["ECO_COD"].isin(ele_eco_full)]
                        .merge(data_loader.eco_anag, on="ECO_COD", how="left")
                        .copy()
                    )
                    df_fit_aggr = data_loader.eco_group_fit_df.copy()
                    df_forecast_eco = (
                        df_rec_forecast_df[config.forecast.spec[-1] + ["DATE_RIF", "forecast_adj"]]
                        .copy()
                        .rename(columns={"DATE_RIF": "ds", "forecast_adj": "y_hat"})
                    )
                    df_forecast_aggr = data_loader.rec_forecast_df[data_loader.rec_forecast_df["ECO_COD"].isna()][
                        config.forecast.spec[-1] + ["DATE_RIF", "y_hat"]
                    ].copy()
                    df_fit_eco["unique_id"] = (
                        df_fit_eco[config.forecast.spec[-1]]
                        .fillna("")
                        .apply(lambda row: "/".join([str(x) for x in row if pd.notnull(x) and str(x) != ""]), axis=1)
                    )
                    df_forecast_eco["unique_id"] = (
                        df_forecast_eco[config.forecast.spec[-1]]
                        .fillna("")
                        .apply(lambda row: "/".join([str(x) for x in row if pd.notnull(x) and str(x) != ""]), axis=1)
                    )
                    ...
                    df_fit_aggr["unique_id"] = (
                        df_fit_aggr[config.forecast.spec[-1]]
                        .fillna("")
                        .apply(lambda row: "/".join([str(x) for x in row if pd.notnull(x) and str(x) != ""]), axis=1)
                    )
                    df_fit_aggr = df_fit_aggr.rename(columns={"DATE_RIF": "ds"})[["unique_id", "ds", "y", "y_hat"]]
                    df_forecast_aggr["unique_id"] = (
                        df_forecast_aggr[config.forecast.spec[-1]]
                        .fillna("")
                        .apply(lambda row: "/".join([str(x) for x in row if pd.notnull(x) and str(x) != ""]), axis=1)
                    )
